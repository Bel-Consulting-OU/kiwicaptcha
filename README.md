...
The Rust crate and the PHP SDK are protocol-identical and share a language-neutral JSON record schema; the PHP test suite pins its implementation to fixtures generated by the Rust crate.
...
| Suite | Count |
|-------|-------|
| Rust core (`packages/kiwicaptcha`) | 73 tests |
| PHP core (`packages/kiwicaptcha-php`) | 102 tests / 195 assertions |
| Symfony bundle (`packages/kiwicaptcha/integrations/symfony`) | 70 tests / 217 assertions |

The PHP and Symfony suites are pinned to fixtures generated by the Rust crate,
so the implementations can never drift apart.
