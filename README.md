...
The full per-feature documentation (protocol v2 signing, the v3 decoy, v4 execution-canonical and v5 rsw-identity extensions, clock policy, attempt accounting, key rotation, revocation) is in [the Rust core documentation](packages/kiwicaptcha/README.md).
...
GitHub Actions is the source of truth for test status: Rust (fmt, clippy with `-D warnings`, tests, wasm32 build), PHP 8.1–8.5 (composer validation + PHPUnit), Symfony 6.4/7.x/8.x (container compilation + KernelBrowser), real-Redis concurrency, and browser specs (widget, security, migration, accessibility).
