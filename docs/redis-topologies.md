...
| `operator_managed` + `ha_authority: pinned_primary` (the `ha_safe` profile) | A direct single-node Predis client (aggregates and phpredis are refused at build under pinned_primary); the PinnedPrimaryAuthorityGuard is initialized explicitly (`kiwicaptcha:ha-initialize` or the `ha_authority_expected` identity; production never auto-pins) and refuses on any change (`{kiwi:<ns>}:authority:pin`), so the operator contract is mechanically enforced | "HA authority": PASS when armed and stable; FAIL on a changed authority or an unarmed guard (see docs/ha-authority.md) |
