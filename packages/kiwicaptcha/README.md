...
- Identity-bearing rsw canonical, protocol v5: an rsw issuance signs the authenticated modulus identity as the final canonical segment and stamps protocol v5. That identity is the canonical-byte sha256 of the decoded 256-byte modulus, exactly the `rsw-keygen`'s `rsw_modulus_n_sha256`. The verifier resolves the trapdoor by that authenticated identity: the rotation keyring first, then the active pair. An accepted proof is therefore always under the exact signed modulus. An unknown identity fails closed with `UnsupportedRswParams` and never falls through to an arbitrary active pair. Identity-bearing records at protocol 2..4 (issued before v5, carrying the legacy base64-text identity) stay verifiable for one bounded migration window. Emission follows the two-phase rule: the explicit `EmissionCapabilities` ceiling must be confirmed at the feature version first, and the plain `issue_challenge` default is the capability-free legacy shape. A pre-v5 verifier rejects the unknown version, so it can never silently ignore the identity. The Symfony controller derives the ceiling from the central `min_protocol_version >= 5` floor.
