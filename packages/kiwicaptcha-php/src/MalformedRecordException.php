...
final class MalformedRecordException extends \RuntimeException
{
    ...
    /**
     * The protocol-v4 `execution_version` is outside the canonical
     * register 1..MAX_EXECUTION_VERSION. Only versions in the register
     * exist in the wire contract; anything else is a corrupt or foreign
     * record.
     */
    public static function invalidExecutionVersion(int $version): self
    {
        return new self(sprintf(
            'record field "execution_version" must be one of the canonical execution-dimension versions 1..%d, got %d',
